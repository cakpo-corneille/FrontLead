...
export default function TrackingPage() {
  ...
  const loadAll = useCallback(
    async (silent = false) => {
      ...
      try {
        const params =
          sessionFilter === 'active' ? '?is_active=true' : '';
        const [ovRes, dayRes, hourRes, topRes, sessRes, routerRes] = await Promise.all([
          fetchWithAuth('/session-analytics/overview/'),
          fetchWithAuth('/session-analytics/by-day/?days=30'),
          fetchWithAuth('/session-analytics/by-hour/'),
          fetchWithAuth('/session-analytics/top-clients/'),
          fetchWithAuth(`/sessions/${params}`),
    ...
    [fetchWithAuth, sessionFilter, toast],
  );
  ...
  const filteredSessions = useMemo(() => {
    // On ne garde que les sessions d'aujourd'hui (minuit à maintenant)
                ...
                <>
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {hasRouters && (
                            s.is_active ? (
                                ...
                                <Badge
                                variant="outline"
                                className="bg-muted text-foreground shrink-0"
                                >
                                Terminée
                                </Badge>
                            )
                  ...
                  >
                    <Table>
                      <TableHeader>
                        <TableRow>
                          {hasRouters && <TableHead>Statut</TableHead>}
                          <TableHead>Client</TableHead>
                          {/* Masquer le champ "Routeur" */}
                          {/* <TableHead>Routeur</TableHead> */}
                          <TableHead>Ticket</TableHead>
                          <TableHead>Plan</TableHead>
                          {hasRouters && <TableHead>Durée / Total</TableHead>}
                          {hasRouters && <TableHead>Conso.</TableHead>}
                                ...
                                <div>
                                  ...
                                  {s.plan_price_fcfa != null && (
                                    <div className="text-[11px] text-muted-foreground">
                                      {formatFcfa(s.plan_price_fcfa)}
                                      {s.plan_duration_minutes && ` · ${s.plan_duration_minutes} min`}
                                    </div>
                                  )}
                                </div>
                              ) : s.session_timeout_seconds && s.session_timeout_seconds > 0 ? (
                                <Badge variant="outline" className="text-[10px] font-normal border-blue-200 bg-blue-50 text-blue-700 animate-pulse">
                                  Identification...
                                </Badge>
                              ) : (
                  ...
                  <TableBody>
                    {topClients.map((c, i) => (
                      <TableRow key={c.client_id}>
                        ...
                        <TableCell className="text-sm">
                          <div className="font-medium">
                            {c.email || c.phone || `Client #${c.client_id}`}
                          </div>
                          {c.email && c.phone && (
