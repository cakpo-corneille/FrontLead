...
export default function ClientsPage() {
  ...
  const fetchClients = useCallback(async (url: string) => {
    if (!fetchWithAuth) { setIsLoading(false); return; }
    setIsLoading(true);
    try {
  ...
  const addTag = () => {
    ...
    if (tag && !sheetTags.includes(tag)) {
      setSheetTags(prev => [...prev, tag]);
    }
    setSheetTagInput('');
  };

  const removeTag = (tag: string) => setSheetTags(prev => prev.filter(t => t !== tag));
  ...
  const handleSaveCRM = async () => {
    ...
    try {
      const response = await fetchWithAuth(`/leads/${selectedLead.id}/`, {
        ...
        body: JSON.stringify({ tags: sheetTags, notes: sheetNotes }),
      });
      if (!response.ok) throw new Error('Échec de la sauvegarde.');
      const updated = await response.json();
      setClients(prev => prev.map(c => c.id === updated.id ? updated : c));
      toast({ title: 'Succès', description: 'Informations CRM mises à jour.' });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: 'Lead supprimé', description: 'Les données ont été supprimées (RGPD).' });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
        <div>
          ...
          <p className="text-muted-foreground">{pagination.count} leads collectés</p>
        </div>
          ...
          {selectedLead && (
            <div className="flex-1 overflow-y-auto py-4 space-y-6">
              <div className="space-y-2 text-sm">
                ...
                {selectedLead.phone && <p><span className="text-muted-foreground">Tél :</span> {selectedLead.phone}</p>}
                <p>
                ...
                <p><span className="text-muted-foreground">Vu le :</span> {format(new Date(selectedLead.last_seen), 'dd MMMM yyyy, HH:mm', { locale: fr })}</p>
              </div>
              ...
              {!selectedLead.is_verified && (
                <Button
                  variant="outline"
                  className="w-full"
                  onClick={() => handleResendOTP(selectedLead)}
                  disabled={resendingIds.has(selectedLead.id)}
                >
                  {resendingIds.has(selectedLead.id)
                    ? <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    : <SendHorizonal className="mr-2 h-4 w-4" />}
