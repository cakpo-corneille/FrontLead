...
export default function DashboardPage() {
  ...
  const dateFrom = useMemo(() => {
    const now = new Date();
    if (period === 'day') return format(startOfDay(now), 'yyyy-MM-dd');
  ...
  useEffect(() => {
    ...
    const fetchAll = async (silent = false) => {
      ...
      try {
        const [sumRes, ovRes, dayRes, sessRes] = await Promise.all([
          fetchWithAuth(`/analytics/summary/?date_from=${dateFrom}`),
          fetchWithAuth(`/tracking-analytics/overview/?date_from=${dateFrom}`),
          fetchWithAuth(`/tracking-analytics/by-day/?date_from=${dateFrom}`),
          fetchWithAuth(`/sessions/?page_size=20&is_active=true&date_from=${new Date().toISOString().split('T')[0]}`),
        ]);
        const [sum, ov, day, sess] = await Promise.all([
          sumRes.json(),
          ovRes.json(),
          dayRes.json(),
          sessRes.json(),
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SheetContent className="sm:max-w-2xl w-full flex flex-col p-0 h-full overflow-hidden">
                          ...
                          <TableCell className="pl-6 font-medium">
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              <span className="truncate">
                                {client.name || client.first_name || client.last_name || client.phone || client.email || `#${index + 1}`}
                              </span>
                            </div>
                          </TableCell>
